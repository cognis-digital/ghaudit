...
[![PyPI](https://img.shields.io/pypi/v/cognis-ghaudit.svg)](https://pypi.org/project/cognis-ghaudit/)
[![CI](https://github.com/cognis-digital/ghaudit/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/ghaudit/actions)
...
GHAUDIT exists for one job — audit a github org's security posture (branch rules, 2fa, secrets) from an export — and does it without a SaaS bill or heavyweight setup.
...
```bash
ghaudit --version
ghaudit scan .                      # scan the current project
ghaudit scan . --format json
ghaudit scan . --fail-on high       # non-zero exit for CI gates
ghaudit mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **legitify**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`ghaudit` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
